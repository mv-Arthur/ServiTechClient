...
import { Chat, GetOrders, StatusType } from "../models/response/OperatorResponse";
import { OperatorService, roleForGetOrdersToOperator } from "../services/OperatorService";
...
class OperatorStore {
	...
	setStatus(orderId: number, status: string, message: string) {
		this.orders = this.orders.map((order) => {
			if (order.id === orderId) {
				const obj = { ...order, status: status as StatusType, message: message };
				return obj;
			}
			return order;
		});
