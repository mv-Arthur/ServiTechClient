        ...
        return (
          currentUser && {
            ...
            order: currentUser.order.map((order) =>
              order.id === id
                ? { ...order, price: price, status: status }
                : order
            ),
          }
        );
        ...
        >
          <Box sx={style}>
            ...
            <Typography id="modal-modal-description" sx={{ mt: 2 }}>
              ВНИМАНИЕ! все заявки со статусом "готово к выдаче" и "отклонено"
              будут удалены из вашей рабочей области и отправлены в бухгалтерию,
              ...
              <Button
                onClick={async () => {
                  if (findFullfiled(orderAdminStore.ordersForUsers)) {
                    ...
                    setCurrentUser((currentUser) => {
                      if (currentUser) {
                        const founded = orderAdminStore.ordersForUsers.find(
                          (user) => {
